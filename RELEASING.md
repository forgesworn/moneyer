...
```bash
npm dist-tag rm @forgesworn/moneyer next
npm deprecate @forgesworn/moneyer@0.1.0-next.0 "bootstrap seed - use 0.1.0 or later"
```
...
The published tarball carries `web/dist` as well as `dist` - the mint serves
its own website from memory, so the built site is part of the package, not a
separate deploy artefact.
